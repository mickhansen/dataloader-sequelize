    ...
    return function batchedGetBelongsTo(instance, options = {}) {
      ...
      return Promise.resolve().then(() => {
        ...
        if (options.paranoid === false) {
          const cacheKey = getCacheKey(this.target, this.targetKey, { paranoid: options.paranoid });
          loader = loaders.autogenerated.get(cacheKey);
          if (!loader) {
            loader = createModelAttributeLoader(this.target, this.targetKey, { paranoid: options.paranoid });
            loaders.autogenerated.set(cacheKey, loader);
          }
        }
