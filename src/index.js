...
function shimModel(target) {
  ...
  // findOne
  shimmer.massWrap(target, methods(Sequelize.version).findOne, original => {
    return function batchedFindOne(options = {}) {
      ...
      if (options.transaction || options.include || activeClsTransaction() || !options[EXPECTED_OPTIONS_KEY] || keys.length !== 1 || !['string', 'number'].includes(typeof id)) {
        return original.apply(this, arguments);
      }
